...
export default function DeliveryTime({ onSelect, initialValue }) {
  ...
  const hasAvailableDays = nextDays.some(
    (dateStr) => isDayEnabled(dateStr) && !isPastDay(dateStr),
  );
                    ...
                    {slots.map((slot) => {
                      ...
                      return (
                        <option key={slot.start} value={slot.start}>
                          {formatHourDisplay(startStr)} –{" "}
                          {formatHourDisplay(endStr)}
                        </option>
                      );
                    })}
                  </select>
